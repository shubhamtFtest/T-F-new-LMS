({
    getSearchProductDataHelper:function(component,event){
        ...
        var lstSelectedSubjectCode=component.get("v.lstSelectedSubjectCode");
        ...
        action.setParams({
            ...
            subjectCode:component.get("v.multiResultCodes"),
            ...
            if (state === 'SUCCESS') {
                ...
                if(datacount !== undefined && datacount>0){
                    if(datacount>2000){
                        ...
                        component.set("v.searchProductTitle","Search Product");
                    }
                    if(datacount<2000){
                        debugger;
    ...
    checkFilterDataValidations:function(component,event){
        ...
        var subjectCodeValues = component.get("v.multiResultCodes");
        ...
        if((isbnValue === '' || isbnValue === undefined) && (productTitle === '' || productTitle === undefined) && (productAuthor==='' || productAuthor === undefined) && (subjectListValues===''||subjectListValues===undefined) && (subjectCodeValues===''||subjectCodeValues===undefined)
    ...
    downloadFileAsCsv:function(component,event,helper){
        ...
        var subjectCode = JSON.parse(JSON.stringify(component.get("v.multiResultCodes")));
        if(subjectCode !== undefined){
            for(var i=0;i<subjectCode.length;i++){
                if(subjectCode[i].includes("&") !== -1 || subjectCode[i].includes(",") !== -1){
                    subjectCode[i]=subjectCode[i].replace(/&/g, "~");
                    subjectCode[i]=subjectCode[i].replace(/,/g, "!");
                }
            }
        } 
        ...
        var poup = window.open('../apex/TF_AG_exportcsv?paramdemo='+encodedParam+'&subjectList='+subjectList+'&subjectCode='+subjectCode+'&lstSelectedMediumData='+lstSelectedMediumData+'&minprice='+minprice+'&maxprice='+maxprice);
    },
    downloadFileAsMultiIsbnCsv:function(component,event,helper){
        ...
        var encodedParam = encodeURIComponent(param);
        var poup = window.open('../apex/TF_AG_exportcsv?paramdemo='+encodedParam+'&multiIsbnListData='+listData);
    ...
    downloadFileAsReadFileCsv:function(component,event,helper){
        ...
        var poup = window.open('../apex/TF_AG_exportcsv?paramdemo='+encodedParam+'&infoList='+infoList);
    },
    ...
    dataFromController : function(component,event,data){
        ...
        var isSortingDescDiscountPercent=component.get("v.isSortingDescDiscountPercent");
                    ...
                    component.set("v.lstSelectedSubjectCode",[]);

({
    ...
    handleChange: function (cmp, evt){
        var accId = cmp.find('accId').value;
        cmp.set('v.myAcctId',accId)
    },
    ...
    searchItemSelected: function (cmp, event, helper) {
        ...
        var record = cmp.get('v.selectedLookUpRecord');
        console.log(record);
        var recId = record.Id;
    ...
    resetAttr:function (cmp, event, helper) {
        cmp.set('v.selectedLookUpRecordId','');
        cmp.set('v.selectedValue','');
    },
    searchButtonSelected: function (cmp, event, helper) {
        
        var recID = cmp.get("v.selectedLookUpRecordId");
        cmp.set("v.recordID", recID);
        ...
        action.setParams({
            accountId: recID,
            channel: cmp.get('v.selectedValue'),
                ...
                if (!$A.util.isEmpty(res.getReturnValue()) && !$A.util.isEmpty(res.getReturnValue().paginatedOrderResult) && !$A.util.isEmpty(res.getReturnValue().paginatedOrderResult.data)) {
                    ...
                    window.setTimeout(
                        $A.getCallback(function () {
                            ...
                            if (totalPageSize <= cmp.get("v.pageno")) {
        ...
        helper.sortBy(cmp, "orderDate");
    },
    
    searchOrderSelected: function (cmp, event, helper) {
        var recID = cmp.get("v.selectedLookUpRecordId");
        cmp.set("v.recordID", recID);
        ...
        console.log("v.recId" + recID);
        var orderId = cmp.find("SearchedOrderId").get("v.value");
        ...
        // API callout  then save in OrdersFetched  getOrdersForCustomer(String accountId)
        var action = cmp.get('c.getSingleOrderForCustomer');
        //action.setStorable();
        ...
        action.setCallback(this, function (res) {
            ...
            if (res.getState() === 'SUCCESS') {
                ...
                if (!$A.util.isEmpty(res.getReturnValue()) && !$A.util.isEmpty(res.getReturnValue().order)) {
                    ...
                    //arr= res.getReturnValue().paginatedOrderResult.data
                    arr.push(res.getReturnValue().order);
                    console.log('total pages -' + res.getReturnValue().metadata.totalPagesize);
                    cmp.set("v.totalPageSize", res.getReturnValue().metadata.totalPagesize);
                    cmp.set("v.addressType", res.getReturnValue().metadata.addresMappg);
                    arr.sort(function (a, b) {
    ...
    sortByAddress: function(component, event, helper) {
        helper.sortBy(component, "billingAddress.addressContactEmail");
    },
    sortBySAPNumber: function(component, event, helper) {
        helper.sortBy(component, "shippingAddress.businessPartnerNo");
    },
    sortByOrderAmount: function(component, event, helper) {
        helper.sortBy(component, "orderPrice");
    },
    sortByOrderDate: function(component, event, helper) {
        helper.sortBy(component, "orderDate");
    },
    sortByOrderStatus: function(component, event, helper) {
        helper.sortBy(component, "orderStatus");
    },
    ...
    prevPage: function(cmp, event, helper) {
        cmp.set("v.pageno", Math.max(cmp.get("v.pageno")-1, 1));
        console.log(cmp.get("v.pageno"));
        
        if (cmp.get("v.totalPageSize") > cmp.get("v.pageno")) {
            cmp.find("nextPgnBtn").set("v.disabled", false);
        }
        var actionToDo = 'prev';
        cmp.set("v.actionToDo", actionToDo);
        
        if(cmp.get("v.pageno")==1){
            cmp.find("previousPgnBtn").set("v.disabled", true);
        }
        helper.setSearchList(cmp, event, helper);
    },
    nextPage: function(cmp, event, helper) {
        console.log(cmp.get("v.pageno"));
        if(cmp.get("v.totalPageSize")>cmp.get("v.pageno")+1){
            cmp.find("previousPgnBtn").set("v.disabled", false);
        }else{
            cmp.find("nextPgnBtn").set("v.disabled", true);
        }
        var actionToDo = 'next';
        cmp.set("v.actionToDo", actionToDo);
        cmp.set("v.pageno", Math.min(cmp.get("v.pageno"), cmp.get("v.totalPageSize")));
        if (cmp.get("v.totalPageSize") == cmp.get("v.pageno")+1) {
            cmp.find("nextPgnBtn").set("v.disabled", true);
        }
        helper.setSearchList(cmp, event, helper);
    ...
    lastPage: function(component, event, helper) {
        component.set("v.pageno", component.get("v.totalPageSize"));
        helper.setSearchList(component, event, helper);
    },
    
    
    onChange: function(cmp,evt,helper){
        console.log('on change');
        var queryTerm = cmp.find('selectedSearch').get('v.value');
        console.log(queryTerm);
        if(queryTerm == '1'){
            cmp.set('v.showOrderInput',true);
            cmp.set('v.showAccInput',false);
        }
        else if(queryTerm == '2'){
            cmp.set('v.showAccInput',true);
        	cmp.set('v.showOrderInput',false);
        }else{
            cmp.set('v.showAccInput',false);
        	cmp.set('v.showOrderInput',false);
        }

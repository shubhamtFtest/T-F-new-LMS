trigger Trigger_Department on Department__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_Department.Trig trig = new TriggerInterface_Department.Trig();
        ...
        if (t.Object__c == 'Department__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Department__c>();
                trig.newList = new List<Department__c>();
                trig.oldMap = new Map<Id,Department__c>();
                trig.newMap = new Map<Id,Department__c>();
                trig.triggerSize = 0;
                for (Integer i=0; i<Trigger.size; i++) {
                    //enable the below if we ever add recordtypes to this object
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_Department.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_Department.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_Department (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_Department (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

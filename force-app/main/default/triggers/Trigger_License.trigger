trigger Trigger_License on License__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
    system.debug('*****in trigger license');
    TriggerInterface_License.Trig trig = new TriggerInterface_License.Trig();
        ...
        if (t.Object__c == 'License__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<License__c>();
                trig.newList = new List<License__c>();
                trig.oldMap = new Map<Id,License__c>();
                trig.newMap = new Map<Id,License__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_License.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_License.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_License (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_License (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

trigger Trigger_UserServicePresence on UserServicePresence (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_UserServicePresence.Trig trig = new TriggerInterface_UserServicePresence.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'UserServicePresence') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<UserServicePresence>();
                trig.newList = new List<UserServicePresence>();
                trig.oldMap = new Map<Id,UserServicePresence>();
                trig.newMap = new Map<Id,UserServicePresence>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_UserServicePresence.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_UserServicePresence.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_UserServicePresence (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_UserServicePresence (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

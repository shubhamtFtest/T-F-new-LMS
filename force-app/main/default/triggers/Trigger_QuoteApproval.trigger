/**
 * Created by Malgorzata on 21-Jul-17.
 */

trigger Trigger_QuoteApproval on Quote_Approval__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
	TriggerInterface_QuoteApproval.Trig trig = new TriggerInterface_QuoteApproval.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'Quote_Approval__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
                ...
                trig.oldList = new List<Quote_Approval__c>();
                trig.newList = new List<Quote_Approval__c>();
                trig.oldMap = new Map<Id,Quote_Approval__c>();
                trig.newMap = new Map<Id,Quote_Approval__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_QuoteApproval.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_QuoteApproval.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_QuoteApproval (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_QuoteApproval (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

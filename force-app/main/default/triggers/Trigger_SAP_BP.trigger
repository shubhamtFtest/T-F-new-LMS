trigger Trigger_SAP_BP on SAP_BP__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_SAP_BP.Trig trig = new TriggerInterface_SAP_BP.Trig();
        ...
        if (t.Object__c == 'SAP_BP__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<SAP_BP__c>();
                trig.newList = new List<SAP_BP__c>();
                trig.oldMap = new Map<Id,SAP_BP__c>();
                trig.newMap = new Map<Id,SAP_BP__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_SAP_BP.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_SAP_BP.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_SAP_BP (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_SAP_BP (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

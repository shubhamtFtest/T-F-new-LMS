trigger Trigger_CSAT on CSAT__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
        ...
        if (t.Object__c == 'CSAT__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<CSAT__c>();
                trig.newList = new List<CSAT__c>();
                trig.oldMap = new Map<Id,CSAT__c>();
                trig.newMap = new Map<Id,CSAT__c>();
                trig.triggerSize = 0;
                for (Integer i=0; i<Trigger.size; i++) {
                    ...
                    //commented the record type condition as there are no record types on CSAT object
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_CSAT.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_CSAT.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_CSAT (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_CSAT (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

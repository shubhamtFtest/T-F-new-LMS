trigger Trigger_SocietyQuote on Society_Quote__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
	TriggerInterface_SocietyQuote.Trig trig = new TriggerInterface_SocietyQuote.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'Society_Quote__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
                ...
                trig.oldList = new List<Society_Quote__c>();
                trig.newList = new List<Society_Quote__c>();
                trig.oldMap = new Map<Id,Society_Quote__c>();
                trig.newMap = new Map<Id,Society_Quote__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_SocietyQuote.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_SocietyQuote.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_SocietyQuote (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_SocietyQuote (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

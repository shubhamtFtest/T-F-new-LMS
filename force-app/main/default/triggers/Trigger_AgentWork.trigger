trigger Trigger_AgentWork on AgentWork (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_AgentWork.Trig trig = new TriggerInterface_AgentWork.Trig();
        ...
        if (t.Object__c == 'AgentWork') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<AgentWork>();
                trig.newList = new List<AgentWork>();
                trig.oldMap = new Map<Id,AgentWork>();
                trig.newMap = new Map<Id,AgentWork>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_AgentWork.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_AgentWork.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_AgentWork (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_AgentWork (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

trigger Trigger_EmailPreference on Email_Preference__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_EmailPreference.Trig trig = new TriggerInterface_EmailPreference.Trig();
...
system.debug('JEFF: 1');
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'Email_Preference__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Email_Preference__c>();
                trig.newList = new List<Email_Preference__c>();
                trig.oldMap = new Map<Id,Email_Preference__c>();
                trig.newMap = new Map<Id,Email_Preference__c>();
                trig.triggerSize = 0;
                 system.debug('JEFF: 2');
                ...
                for (Integer i=0; i<Trigger.size; i++) {
                    ...
                    //this object doesn't have recordtypes
                        trig.triggerSize++;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_EmailPreference.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_EmailPreference.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_EmailPreference (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_EmailPreference (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

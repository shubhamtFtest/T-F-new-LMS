trigger Trigger_Billing on Billing__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_Billing.Trig trig = new TriggerInterface_Billing.Trig();
        ...
        if (t.Object__c == 'Billing__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Billing__c>();
                trig.newList = new List<Billing__c>();
                trig.oldMap = new Map<Id,Billing__c>();
                trig.newMap = new Map<Id,Billing__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_Billing.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_Billing.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_Billing (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_Billing (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

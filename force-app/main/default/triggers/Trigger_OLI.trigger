trigger Trigger_OLI on OpportunityLineItem (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_OLI.Trig trig = new TriggerInterface_OLI.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'OpportunityLineItem') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<OpportunityLineItem>();
                trig.newList = new List<OpportunityLineItem>();
                trig.oldMap = new Map<Id,OpportunityLineItem>();
                trig.newMap = new Map<Id,OpportunityLineItem>();
                trig.triggerSize = 0;
                for (Integer i=0; i<Trigger.size; i++) {
                    //if (!mapTriggerRecordtypes.containsKey(t.Name) || mapTriggerRecordtypes.get(t.Name).containsKey((trigger.isDelete ? trigger.old : trigger.new)[i].OpportunityRecordtypeId__c)) {
                    if (t.Run_for_Recordypes__c == 'All' || (mapTriggerRecordtypes.containsKey(t.Name) && mapTriggerRecordtypes.get(t.Name).containsKey((trigger.isDelete ? trigger.old : trigger.new)[i].OpportunityRecordtypeId__c))) {
                        trig.triggerSize++;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_OLI.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_OLI.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_OLI (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_OLI (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

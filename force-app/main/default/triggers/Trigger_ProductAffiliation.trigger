trigger Trigger_ProductAffiliation on Product_Affiliation__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
	TriggerInterface_ProductAffiliation.Trig trig = new TriggerInterface_ProductAffiliation.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'Product_Affiliation__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
                ...
                trig.oldList = new List<Product_Affiliation__c>();
                trig.newList = new List<Product_Affiliation__c>();
                trig.oldMap = new Map<Id,Product_Affiliation__c>();
                trig.newMap = new Map<Id,Product_Affiliation__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_ProductAffiliation.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_ProductAffiliation.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_ProductAffiliation (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_ProductAffiliation (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

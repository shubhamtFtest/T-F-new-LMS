trigger Trigger_ICR on Informa_Contact_Role__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_ICR.Trig trig = new TriggerInterface_ICR.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'Informa_Contact_Role__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Informa_Contact_Role__c>();
                trig.newList = new List<Informa_Contact_Role__c>();
                trig.oldMap = new Map<Id,Informa_Contact_Role__c>();
                trig.newMap = new Map<Id,Informa_Contact_Role__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_ICR.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_ICR.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_ICR (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_ICR (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

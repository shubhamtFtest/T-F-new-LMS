trigger Trigger_Error_Log_Items on Error_Log_Items__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_Error_Log_Items.Trig trig = new TriggerInterface_Error_Log_Items.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'Error_Log_Items__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<Error_Log_Items__c>();
                trig.newList = new List<Error_Log_Items__c>();
                trig.oldMap = new Map<Id,Error_Log_Items__c>();
                trig.newMap = new Map<Id,Error_Log_Items__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_Error_Log_Items.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_Error_Log_Items.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_Error_Log_Items__c (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_Error_Log_Items__c (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

trigger Trigger_AccountAffiliation on Account_Affiliation__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
    TriggerInterface_AccountAffiliation.Trig trig = new TriggerInterface_AccountAffiliation.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'Account_Affiliation__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
                ...
                trig.oldList = new List<Account_Affiliation__c>();
                trig.newList = new List<Account_Affiliation__c>();
                trig.oldMap = new Map<Id,Account_Affiliation__c>();
                trig.newMap = new Map<Id,Account_Affiliation__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_AccountAffiliation.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_AccountAffiliation.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                if (!Test.isRunningTest() && (t.Recursive_Execution_Allowed__c == null || !t.Recursive_Execution_Allowed__c))
                                    ...
                                    system.debug('********** Trigger_AccountAffiliation (' + executionType + ') Start: ' + t.Name + ' **********');
                                    Datetime startTime = Datetime.now();
                                    runnableClass.run(trig);
                                    DateTime endTime = Datetime.now();
                                    Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                    system.debug('********** Trigger_AccountAffiliation (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                    if (seconds > 10)

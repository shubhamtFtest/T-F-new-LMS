trigger Trigger_ProductInterest on ProductInterest__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_ProductInterest.Trig trig = new TriggerInterface_ProductInterest.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'ProductInterest__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<ProductInterest__c>();
                trig.newList = new List<ProductInterest__c>();
                trig.oldMap = new Map<Id,ProductInterest__c>();
                trig.newMap = new Map<Id,ProductInterest__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_ProductInterest.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_ProductInterest.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_ProductInterest (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_ProductInterest (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

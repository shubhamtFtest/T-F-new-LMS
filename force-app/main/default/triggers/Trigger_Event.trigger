trigger Trigger_Event on Event (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

	TriggerInterface_Event.Trig trig = new TriggerInterface_Event.Trig();
		...
		if (t.Object__c == 'Event') {
			if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
			 ...
			 || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
				trig.oldList = new List<Event>();
				trig.newList = new List<Event>();
				trig.oldMap = new Map<Id,Event>();
				trig.newMap = new Map<Id,Event>();
				trig.triggerSize = 0;
				...
				if (trig.triggerSize > 0) {
					...
					if (runnableType != null) {
						TriggerInterface_Event.Runnable runnableClass;
						try {
							runnableClass = (TriggerInterface_Event.Runnable) runnableType.newInstance();
						} catch(Exception e) {
						...
						if (runnableClass != null) {
							...
							system.debug('********** Trigger_Event (' + executionType + ') Start: ' + t.Name + ' **********');
							Datetime startTime = Datetime.now();
							runnableClass.run(trig);
							DateTime endTime = Datetime.now();
							Long seconds = (endTime.getTime() - startTime.getTime())/1000;
							system.debug('********** Trigger_Event (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
							if (seconds > 10)

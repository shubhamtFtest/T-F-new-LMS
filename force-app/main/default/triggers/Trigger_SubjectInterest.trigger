trigger Trigger_SubjectInterest on SubjectInterest__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {

    TriggerInterface_SubjectInterest.Trig trig = new TriggerInterface_SubjectInterest.Trig();
    ...
    for (Trigger__c t : listTriggers) {
        ...
        if (t.Object__c == 'SubjectInterest__c') {
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
             ...
             || (trigger.isAfter && trigger.isUnDelete && t.After_UnDelete__c)) {
                trig.oldList = new List<SubjectInterest__c>();
                trig.newList = new List<SubjectInterest__c>();
                trig.oldMap = new Map<Id,SubjectInterest__c>();
                trig.newMap = new Map<Id,SubjectInterest__c>();
                trig.triggerSize = 0;
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_SubjectInterest.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_SubjectInterest.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_SubjectInterest (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_SubjectInterest (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)

trigger Trigger_SBQQ_QuoteLine on SBQQ__QuoteLine__c (before insert, before update, before delete, after insert, after update, after delete, after undelete) {
    TriggerInterface_SBQQ_QuoteLine.Trig trig = new TriggerInterface_SBQQ_QuoteLine.Trig();
            ...
            if ((trigger.isBefore && trigger.isInsert && t.Before_Insert__c)
                ...
                if (trig.triggerSize > 0) {
                    ...
                    if (runnableType != null) {
                        TriggerInterface_SBQQ_QuoteLine.Runnable runnableClass;
                        try {
                            runnableClass = (TriggerInterface_SBQQ_QuoteLine.Runnable) runnableType.newInstance();
                        } catch(Exception e) {
                        ...
                        if (runnableClass != null) {
                            ...
                            if (!TriggerInterfaceHelper.triggerAlreadyRan.contains(t.Name + ':' + executionType)) {
                                ...
                                system.debug('********** Trigger_SBQQ__QuoteLine (' + executionType + ') Start: ' + t.Name + ' **********');
                                Datetime startTime = Datetime.now();
                                runnableClass.run(trig);
                                DateTime endTime = Datetime.now();
                                Long seconds = (endTime.getTime() - startTime.getTime())/1000;
                                system.debug('********** Trigger_SBQQ__QuoteLine (' + executionType + ') Finished in ' + seconds + ' seconds: ' + t.Name + ' **********');
                                if (seconds > 10)
